...
"""
Script Principal de Análise de Dados - TransDevs Data Analysis (VERSÃO FINAL FASE 2)
Adiciona a geração de uma análise de crescimento da base de pessoas inscritas.
...
def processar_dados_inscricoes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    estado_variacoes = {'SP': ['são paulo', 'sp', 'sao paulo', 'sãopaulo'], 'RJ': ['rio de janeiro', 'rj', 'eio de janeiro', 'rio de janeieo'], 'MG': ['minas gerais', 'mg', 'minaa gerais', 'bh'], 'BA': ['bahia', 'ba'], 'CE': ['ceará', 'ce', 'ceara'], 'PE': ['pernambuco', 'pe', 'recife'], 'PR': ['paraná', 'pr', 'parana'], 'RS': ['rio grande do sul', 'rs'], 'SC': ['santa catarina', 'sc'], 'GO': ['goiás', 'go', 'goias', 'goiânia'], 'DF': ['distrito federal', 'df', 'brasília'], 'AM': ['amazonas'], 'RO': ['rondônia'], 'RN': ['rio grande do norte', 'rn'], 'AL': ['alagoas'], 'ES': ['espirito santo', 'esporo santo', 'es'], 'PA': ['pará', 'para'], 'MA': ['maranhão', 'ma'], 'SE': ['sergipe'], 'PI': ['piauí', 'piaui'], 'MS': ['mato grosso do sul', 'ms'], 'MT': ['mato grosso', 'mt'], 'PB': ['paraíba', 'paraiba'], 'AC': ['acre'], 'TO': ['tocantins'], 'RR': ['roraima'], 'Internacional': ['portugal', 'lisboa', 'espanha', 'oizumi']}
    ...
    bins = [0, 17, 24, 34, 44, 54, 64, 150]; labels = ['Menor de 18', '18-24 anos', '25-34 anos', '35-44 anos', '45-54 anos', '55-64 anos', '65+ anos']
    df_anon['faixa_etaria'] = pd.cut(df_anon['idade'], bins=bins, labels=labels, right=False)
    colunas_a_manter = list(dict.fromkeys([col for col in df.columns] + ['person_id', 'computador_acesso', 'estado_padronizado', 'regiao', 'etnia_padronizada', 'genero_padronizado', 'idade', 'faixa_etaria']))
    colunas_a_remover = ['nascdt_temp', 'nascdt_dt', 'estado', 'etnia', 'genero', 'email', 'id', 'nome_completo', 'nascdt', 'telefone', 'nome_primeiro', 'nome_ultimo', 's_link', 'computador']
...
def descobrir_personas_com_clustering(df: pd.DataFrame) -> pd.DataFrame:
    ...
    categorical_features = ['faixa_etaria', 'professional_level_padronizado', 'working']; numerical_features = ['idade']
...
def main():
    logger.info("="*50 + "\n==  INICIANDO PIPELINE DE DADOS COMPLETO (FINAL FASE 2)  ==" + "\n" + "="*50)
    df_inscricoes_raw = carregar_dados(RAW_INSCRICOES_PATH); df_profile_raw = carregar_dados(RAW_PROFILE_PATH); df_voluntariado_raw = carregar_dados(RAW_VOLUNTARIADO_PATH)
